...
const ELO_CONFIG = {
    ...
    // Ganho por ação (dividido por 10 no cálculo para evitar salto brusco)
    ganhos: {
        stream_play_my: 2,
        view_youtube: 1,
        stream_spotify: 3,
    ...
    // Limites de ganho por categoria (evita ELO inflado)
    limites: {
        play_my_max: 300,
        externos_max: 400,
...
module.exports = async (req, res) => {
    ...
    const { action, music_id } = params;

    console.log(`⚡ [elo] ${action}`);
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            message: '⚡ PLAY MY ELO API',
            version: '1.0.0',
            acoes: [
                '?action=calcular&music_id=X',
                '?action=atualizar_todos',
                '?action=ranking',
