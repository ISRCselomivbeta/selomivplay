...
async function loadPortfolio(silent = false) {
    ...
    try {
        ...
        if (result?.success && result?.data) {
            ...
            else if (result.data && typeof result.data === 'object') { state.portfolioAssets = result.data.investimentos || []; }
            else { state.portfolioAssets = []; }
        } else { state.portfolioAssets = []; }
    } catch (error) { console.error('Erro ao carregar portfólio:', error); state.portfolioAssets = []; }
    renderPortfolio();
...
function renderPortfolio() {
    ...
    assets.forEach(asset => {
        ...
        if (music && music.valor_acao) { valorAtualTotal += (asset.quantidade || 0) * music.valor_acao; }
        else { valorAtualTotal += valorInvestido; }
    });
    ...
    container.innerHTML += assets.map((asset) => {
        ...
        else if (music.link_youtube) { const videoId = extractYouTubeId(music.link_youtube); if (videoId) coverImage = `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`; }
        const musicIndex = state.playlist.findIndex(m => m.id === music.id);
        return `<div class="spotify-card" onclick="playTrack(${musicIndex})">
            <div class="spotify-cover"><img src="${coverImage}" alt="${music.titulo || 'Música'}" onerror="this.src='${PLACEHOLDERS.MIV_300}'"><div class="play-overlay" onclick="event.stopPropagation(); playTrack(${musicIndex})"><i class="bi bi-play-fill"></i></div></div>
            <h3 class="spotify-title">${music.titulo || 'Sem título'}</h3>
            <p class="spotify-artist">${music.artista || 'Artista'}</p>
            <div class="spotify-stats"><span class="spotify-elo">${quantidade} ações</span><span class="spotify-price">${formatCurrency(valorAtual)}</span></div>
            <div style="margin-top: 8px;"><span class="badge ${profitVal >= 0 ? 'badge-success' : 'badge-danger'}">${profitVal >= 0 ? '+' : ''}${profitPerc.toFixed(1)}%</span></div>
            <button class="btn-invest" onclick="event.stopPropagation(); openTradeModal({ music_id: '${asset.music_id}', music_title: '${music.titulo || ''}', artist: '${music.artista || ''}', quantidade: ${quantidade}, valor_acao: ${music.valor_acao || 0}, valor_investido: ${valorInvestido} })"><i class="bi bi-arrow-left-right me-1"></i> NEGOCIAR</button>
