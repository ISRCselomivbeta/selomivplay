...
function getFallbackData(action, data) {
  ...
  if (action === 'get_following') return { success: true, data: [] };
  if (action === 'get_carteira') return { success: true, data: { investimentos: [], total_investido: 0, valor_atual: 0, quantidade_itens: 0 } };
...
async function handleRegister() {
  ...
  try {
    const r = await callAPI('register', { nome: name, email: email, senha: password, tipo: type, workLink: link, confirm_url: CONFIG.CONFIRM_EMAIL_URL, accepted_terms: true, terms_version: '8.4.0', accepted_marketing: acceptMkt });
    if (r && r.success) {
...
async function initializeApp() {
  ...
  loadNewsFeed();
  if (typeof loadYouTubeAPI === 'function') loadYouTubeAPI();
  ...
  try {
    ...
    if (r && r.success && Array.isArray(r.data)) {
      state.globalPlaylists = r.data.map(p => ({
        ...
        musicas: Array.isArray(p.musicas) ? p.musicas.map(String) : (typeof p.musicas === 'string' ? p.musicas.split(',').filter(Boolean) : []),
...
async function checkStreamingProgress() {
  ...
  try {
    ...
    if (typeof updatePlayerProgress === 'function') updatePlayerProgress();
...
document.addEventListener('DOMContentLoaded', async () => {
  console.log('🚀 PLAY MY v' + CONFIG.VERSION + ' — Vercel KV primário + GAS fallback');
  HealthCheck.runAll().catch(() => {});
  setInterval(() => HealthCheck.runAll(), 120000);
  ...
  setTimeout(() => { if (typeof loadNewsFeed === 'function') loadNewsFeed(); }, 1500);
