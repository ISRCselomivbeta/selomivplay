// ============================================================
// APP - PLAY MY v8.3 (GAS PRIMÁRIO + VERCEL STANDBY)
// ============================================================
...
const HealthCheck = {
  ...
  async testGAS() {
    try {
      ...
      console.warn('⚠️ GAS offline:', e.message);
    }
  ...
  async runAll() {
    console.log('🔍 Health Check (GAS primário + Vercel standby)...');
    this.mode = 'checking';
    this.updateBadge();
    const gasOk = await this.testGAS();
    if (gasOk) {
      this.mode = 'gas';
      console.log('✅ GAS online — usando dados reais da planilha');
    } else {
      const vercelOk = await this.testVercel();
      if (vercelOk) {
        this.mode = 'vercel';
        console.warn('⚠️ GAS offline — usando Vercel (standby)');
        showToast('⚠️ Backend principal offline. Usando modo standby.', 'warning', 5000);
      } else {
        this.mode = 'fallback';
        console.error('❌ GAS e Vercel offline — modo fallback local');
  ...
  updateBadge() {
    ...
    const modes = {
      ...
      gas:      { i: '✅', t: 'Online', c: '#34c759', bg: 'rgba(52,199,89,0.15)' },
      vercel:   { i: '⚠️', t: 'Standby', c: '#ffcc00', bg: 'rgba(255,204,0,0.15)' },
...
async function callAPI(action, data, _retry) {
  ...
  // 1. GAS (primário)
  if (HealthCheck.mode === 'gas' || HealthCheck.mode === 'checking') {
    try {
  ...
  // 2. Vercel (standby)
  if (HealthCheck.mode === 'vercel' || HealthCheck.vercel.online) {
    try {
      const ctrl = new AbortController();
      ...
      if (r.ok) {
        ...
        if (json && json.success !== false) {
          console.log('📦 [' + action + '] via Vercel (standby)');
          return json;
        }
      }
    } catch (e) {
      console.warn('⚠️ Vercel também falhou [' + action + ']:', e.message);
    }
  }
...
function getFallbackData(action, data) {
  ...
  if (action === 'get_saldo') return { success: true, data: { saldo_disponivel: 0, saldo_bloqueado: 0 } };
...
async function handleRegister() {
  ...
  try {
    const r = await callAPI('register', { nome: name, email: email, senha: password, tipo: type, workLink: link, confirm_url: CONFIG.CONFIRM_EMAIL_URL, accepted_terms: true, terms_version: '8.3.0', accepted_marketing: acceptMkt });
    if (r && r.success) {
...
async function resendConfirmationEmail() {
  ...
  try {
    const r = await callAPI('resend_confirmation', { email: email, confirm_url: CONFIG.CONFIRM_EMAIL_URL });
    if (r && r.success) showToast('✉️ Novo link enviado!', 'success', 5000);
    else showToast((r && r.message) || 'Erro', 'error');
...
function openResetPasswordModal() {
  document.getElementById('resetEmailField').value = document.getElementById('loginEmailField').value || '';
...
function logout() {
  ...
  state.currentUser = null; state.userBalance = 0; state.seloCoinBalance = 0;
  state.playlist = []; state.externalPlaylist = []; state.portfolioAssets = []; state.ledgerData = [];
  state.favoriteMusicIds = []; state.followingArtists = []; state.currentTrackIndex = -1; state.isPlaying = false;
...
async function updateBalanceDisplay() {
  ...
  try {
    const r = await callAPI('get_saldo', { user_id: state.currentUser.id });
...
function updateUserInterface() {
  ...
  if (b) {
    b.textContent = state.currentUser.tipo === 'admin' ? 'Admin' : (state.currentUser.tipo === 'artista' ? 'Artista' : 'Ouvinte');
    b.style.background = state.currentUser.tipo === 'admin' ? 'var(--apple-purple)' : (state.currentUser.tipo === 'artista' ? 'var(--apple-blue)' : 'var(--apple-green)');
  }
...
async function loadMarketplace() {
  try {
    ...
    if (r && r.success && r.data && r.data.length > 0) state.playlist = r.data;
    else if (!state.playlist || state.playlist.length === 0) state.playlist = getFallbackData('get_musicas').data;
  } catch (e) {
    if (!state.playlist || state.playlist.length === 0) state.playlist = getFallbackData('get_musicas').data;
  }
  renderMarketplace();
}

async function loadExternalMarketplace(force) {
  if (!force) {
    const c = localStorage.getItem('miv_external_playlist');
    const t = localStorage.getItem('miv_external_timestamp');
    if (c && t && (Date.now() - parseInt(t)) < 600000) {
      try { state.externalPlaylist = JSON.parse(c); renderExternalMarketplace(); return; } catch (e) {}
    }
  }
  try {
    ...
    if (r && r.success && r.data) {
      ...
      localStorage.setItem('miv_external_playlist', JSON.stringify(r.data));
...
async function loadTopInvestments() {
  try {
    ...
    if (r && r.success && r.data) state.topInvestments = r.data;
  } catch (e) {}
...
async function loadPortfolio() {
  if (!state.currentUser) return;
  try {
    const r = await callAPI('get_carteira', { user_id: state.currentUser.id });
    if (r && r.success && r.data) state.portfolioAssets = Array.isArray(r.data) ? r.data : (r.data.investimentos || []);
  } catch (e) {}
  renderPortfolio(); updatePortfolioValue();
}

async function loadLedger() {
  if (!state.currentUser) return;
  try {
    const r = await callAPI('get_extrato', { user_id: state.currentUser.id });
    if (r && r.success && r.data) state.ledgerData = r.data;
  } catch (e) {}
  renderLedger();
}
...
async function loadUserPlaylists() {
  ...
  try {
    const r = await callAPI('get_playlists', { user_id: state.currentUser.id });
    if (r && r.success && r.data) state.userPlaylists = r.data;
  } catch (e) {}
  renderPlaylists();
}
...
async function loadArtists(force) {
  try {
    ...
    if (r && r.success && r.data) state.artists = r.data;
    else state.artists = getFallbackData('get_artists').data;
  } catch (e) { state.artists = getFallbackData('get_artists').data; }
  renderArtists(); renderFeaturedArtists();
}

async function loadFollowingArtists() {
  if (!state.currentUser) { state.followingArtists = []; return; }
  try {
    const r = await callAPI('get_following', { user_id: state.currentUser.id });
    if (r && r.success && r.data) state.followingArtists = Array.isArray(r.data) ? r.data.map(x => String(x)) : [];
  } catch (e) { state.followingArtists = state.followingArtists || []; }
}

async function loadTickets(force) {
  try {
    const r = await callAPI('get_tickets');
    if (r && r.success && r.data) state.tickets = r.data;
  } catch (e) { state.tickets = state.tickets || []; }
  renderTickets();
}
...
async function loadAdminData() {
  ...
  try {
    ...
    if (r && r.success && r.data) {
      ...
      document.getElementById('adminUsersCount').textContent = r.data.total_usuarios || 0;
      document.getElementById('adminMusicsCount').textContent = r.data.total_musicas || 0;
      document.getElementById('adminGlobalPlaylistsCount').textContent = (state.globalPlaylists || []).length;
      document.getElementById('adminSeloCirculation').textContent = new Intl.NumberFormat('pt-BR').format(r.data.total_investido || 0);
    }
  } catch (e) {}
}
...
function renderExternalMarketplace() {
  ...
  if (!state.externalPlaylist || state.externalPlaylist.length === 0) {
    c.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--apple-label-2)"><i class="bi bi-globe" style="font-size:48px;color:var(--apple-orange)"></i><h3>Nenhuma música externa</h3></div>';
    return;
  }
...
function renderTopInvestments() {
  ...
  if (!state.topInvestments || state.topInvestments.length === 0) {
...
function renderPortfolio() {
  ...
  if (!a.length) {
    c.innerHTML = '<div class="empty-state-actionable" style="grid-column:1/-1"><i class="bi bi-briefcase empty-icon"></i><h5 class="text-muted">Nenhum investimento</h5></div>';
    return;
  }
  c.innerHTML = a.map(x => {
    const m = state.playlist.find(y => y.id === x.music_id) || {};
    return '<div class="spotify-card"><div class="spotify-cover"><img src="' + getCoverUrl(m, false) + '" onerror="this.onerror=null;this.src=\'' + PLACEHOLDERS.MIV_300 + '\'"></div><h3 class="spotify-title">' + (m.titulo || 'Música') + '</h3><p class="spotify-artist">' + (m.artista || '') + '</p><div class="spotify-stats"><span class="spotify-elo">' + (x.quantidade || 0) + ' ações</span><span class="spotify-price">' + formatCurrency(x.valor_total || 0) + '</span></div></div>';
  }).join('');
}

function updatePortfolioValue() {
  const el = document.getElementById('portfolioValue'); if (!el) return;
  let t = 0;
  (state.portfolioAssets || []).forEach(a => {
    const m = state.playlist.find(x => x.id === a.music_id);
    if (m && m.valor_acao) t += (a.quantidade || 0) * m.valor_acao;
    else t += a.valor_total || 0;
  });
...
function renderLedger() {
  ...
  c.innerHTML = sorted.map(t => {
    ...
    return '<tr><td>' + formatDate(t.data) + '</td><td>' + (t.descricao || t.tipo) + '</td><td class="text-end ' + (isNeg ? 'text-danger' : 'text-success') + '"><strong>' + (isNeg ? '-' : '+') + formatCurrency(Math.abs(t.valor || 0)) + '</strong></td><td>' + (t.blockchain_hash ? '⛓️' : '-') + '</td></tr>';
  }).join('');
}
...
function renderPlaylists() {
  ...
  if (pc) {
    if (state.userPlaylists && state.userPlaylists.length) {
      pc.innerHTML = state.userPlaylists.map(p => '<div class="playlist-item"><div class="playlist-cover"><i class="bi bi-music-note-list"></i></div><div><h6 class="mb-0">' + p.nome + '</h6></div></div>').join('');
    } else pc.innerHTML = '<div class="empty-state-actionable"><i class="bi bi-music-note-list empty-icon"></i><h5 class="text-muted">Nenhuma playlist</h5></div>';
  }
  if (fc) {
    ...
    if (favs.length) {
      fc.innerHTML = favs.map(t => '<div class="spotify-card"><div class="spotify-cover"><img src="' + getCoverUrl(t, false) + '"></div><h3 class="spotify-title">' + t.titulo + '</h3></div>').join('');
...
function openInvestModal(i) {
  if (i < 0 || i >= state.playlist.length) return;
  const t = state.playlist[i]; state.currentInvestTrack = t;
...
function openInvestExternalModal(i) {
  if (i < 0 || i >= state.externalPlaylist.length) return;
  state.currentExternalTrack = state.externalPlaylist[i];
...
function updateExternalInvestmentTotal() {
  ...
  document.getElementById('investExternalTotalPriceDisplay').textContent = formatCurrency(t);
}

function adjustExternalQuantity(a) {
...
function openWithdrawalModal() {
  if (state.userBalance < 10) { showToast('Saldo mínimo R$ 10', 'warning'); return; }
...
function processBalanceAdd() {
  const a = parseFloat(document.getElementById('balanceAmountField').value) || 0;
  if (a < 10) { showToast('Mínimo R$ 10', 'error'); return; }
  window.open(CONFIG.MERCADO_PAGO_LINK, '_blank');
  closeModal('addBalanceModal');
  showToast('Finalize o pagamento e aguarde a confirmação.', 'info', 6000);
}
...
async function requestWithdrawal() {
  ...
  if (amount < 10 || amount > state.userBalance || !method || !details) { showToast('Verifique os campos', 'error'); return; }
  const btn = document.getElementById('requestWithdrawalBtn');
  btn.disabled = true;
  try {
    const r = await callAPI('request_withdrawal', { valor: amount, metodo: method, dados_bancarios: details });
    if (r && r.success) {
      if (r.data && r.data.novo_saldo !== undefined) state.userBalance = r.data.novo_saldo;
      else state.userBalance -= amount;
      updateBalanceDisplay();
      await loadLedger();
...
async function submitExternalMusic() {
  ...
  try {
    const r = await callAPI('suggest_external_music', { link_youtube: y, titulo: t, artista: a, valor_acao: p });
    if (r && r.success) {
      showToast('Sugerida!', 'success');
...
async function createPlaylist() {
  ...
  try {
    const r = await callAPI('create_playlist', { nome: name, publica: document.getElementById('playlistPublicField').checked, user_id: state.currentUser.id });
    if (r && r.success) { showToast('Playlist criada!', 'success'); closeModal('createPlaylistModal'); await loadUserPlaylists(); }
  } catch (e) { showToast('Erro', 'error'); }
}
...
async function searchYouTubeDirect(query) {
  ...
  } catch (e) { console.warn('Erro na busca YouTube:', e); }
  return [];
}
...
async function analisarVideoYouTube() {
  ...
  try {
    ...
    if (r && r.success && r.data) {
      ...
      showToast('Erro ao analisar vídeo', 'error');
    }
...
async function loadUserFavorites() {
  ...
  if (cached) { try { const p = JSON.parse(cached); if (Array.isArray(p)) state.favoriteMusicIds = p; } catch (e) {} }
}

function renderMyMusic(tab) {
  const c = document.getElementById('myMusicContent'); if (!c) return;
  c.innerHTML = '<div class="empty-state-actionable" style="grid-column:1/-1"><i class="bi bi-music-note-beamed empty-icon"></i><h5 class="text-muted">Nenhuma música</h5></div>';
}
    ...
    if (s === 1 && c >= 30 && state.currentTrackIndex >= 0 && state.currentTrackIndex < state.playlist.length) {
      ...
      if (now - state.streamingLastReward > 29000) {
        ...
        if (state.currentUser && !localStorage.getItem(key)) {
          ...
          try {
            const r = await callAPI('register_streaming', { music_id: t.id, duration: Math.floor(c), total_duration: 0 });
            if (r && r.success && r.data && r.data.reward) {
              state.seloCoinBalance = (state.seloCoinBalance || 0) + r.data.reward;
              updateBalanceDisplay();
              showToast('🪙 +' + r.data.reward + ' SELO COIN por "' + t.titulo + '"', 'success');
            }
          } catch (e) {}
        }
...
document.addEventListener('DOMContentLoaded', async () => {
  console.log('🚀 PLAY MY v' + CONFIG.VERSION + ' — GAS Primário + Vercel Standby');
  HealthCheck.runAll().catch(() => {});
  setInterval(() => HealthCheck.runAll(), 60000);
...
window.searchYouTubeDirect = searchYouTubeDirect;
