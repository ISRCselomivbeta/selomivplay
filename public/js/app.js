...
// ============ HEALTH CHECK COM FALLBACK INTELIGENTE ============
// GAS é o backend REAL. Vercel é STANDBY.
// Se GAS cair, o app continua funcionando com dados mock/standby.
const HealthCheck = {
  gas: { online: null },
  vercel: { online: null },
  mode: 'checking', // 'gas' | 'vercel' | 'fallback' | 'checking'
  lastCheck: 0,
  checkInterval: 60000, // revalida a cada 60s
  ...
  async runAll() {
    ...
    // 1. Tenta GAS primeiro (é a fonte da verdade)
    ...
    if (gasOk) {
      ...
      // 2. GAS caiu → tenta Vercel como standby
      ...
      if (vercelOk) {
        ...
        // 3. Nada funciona → modo fallback local
        this.mode = 'fallback';
...
const state = {
  ...
  newsLikes: {},
  connectionMode: 'checking' // 'gas' | 'vercel' | 'fallback'
};
...
// ============ CALL API COM ESTRATÉGIA GAS → VERCEL → FALLBACK ============
async function callAPI(action, data, _retry) {
  ...
  // 1. GAS (primário — fonte da verdade)
    ...
    try {
      ...
      if (r.ok) {
        ...
        try { json = JSON.parse(text); } catch (e) {
          // Tenta extrair JSON se vier algo antes (ex: BOM ou HTML)
      ...
      // Marca GAS como offline e tenta Vercel
...
// ============ INICIALIZAÇÃO ============
// (mantém o resto igual ao app.js atual a partir de "// ============ AUTH ============")
