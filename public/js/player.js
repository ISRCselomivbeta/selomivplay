...
function onPlayerReady(event) {
    ...
    try {
        ...
        console.warn('Erro ao setar volume:', e);
    }
    ...
    if (state.isPlaying) {
        try {
            ...
            console.warn('Erro ao iniciar reprodução:', e);
        }
    }
...
function onPlayerError(event) {
    ...
    const errorMessages = {
        ...
        101: 'Embedding não permitido pelo proprietário',
...
function playTrack(index) {
    ...
    if (playerAlbumArt) {
        let coverUrl = CONFIG.PLACEHOLDERS.MIV_56;
        ...
        playerAlbumArt.onerror = function() {
            this.src = CONFIG.PLACEHOLDERS.MIV_56;
        };
    }
...
function playExternalTrack(index) {
    ...
    if (track) {
        ...
        setTimeout(() => {
            playQueue.findAndAddSimilar(track);
        }, 2000);
    }
    ...
    if (playerAlbumArt) {
        let coverUrl = CONFIG.PLACEHOLDERS.EXT_56;
        ...
        playerAlbumArt.src = coverUrl;
        playerAlbumArt.onerror = function() {
            this.src = CONFIG.PLACEHOLDERS.EXT_56;
        };
    }
...
function updateExpandedPlayer() {
    ...
    if (expandedAlbumArt) {
        let coverUrl = isExternal ? CONFIG.PLACEHOLDERS.EXT_300 : CONFIG.PLACEHOLDERS.MIV_300;
        ...
        expandedAlbumArt.onerror = function() {
            this.src = isExternal ? CONFIG.PLACEHOLDERS.EXT_300 : CONFIG.PLACEHOLDERS.MIV_300;
        };
    }
...
function setupMediaSession(track, isExternal) {
    ...
    try {
        let artwork = isExternal ? CONFIG.PLACEHOLDERS.EXT_300 : CONFIG.PLACEHOLDERS.MIV_300;
        ...
        navigator.mediaSession.metadata = new MediaMetadata({
            ...
            artwork: [
                { src: artwork, sizes: '300x300', type: 'image/jpeg' }
            ]
        });
        ...
        console.log('Media Session API indisponível:', e);
    }
}
...
function checkStreamingProgress() {
    ...
    try {
        ...
        const loadedFraction = state.youtubePlayer.getVideoLoadedFraction ? 
            state.youtubePlayer.getVideoLoadedFraction() : 0;
        ...
        console.error('Erro no monitoramento de streaming:', error);
    }
}
